...
interplot.glmerMod <- function(m,
  ...
  if (factor_v1) {
    ...
    if (predPro == TRUE) {
      ...
      pp <-
        rowMeans(plogis(data.matrix(fake_data) %*% t(data.matrix(m.sims@fixef))))
      ...
      pp_bounds <-
        row_quantiles(plogis(data.matrix(fake_data) %*% t(data.matrix(m.sims@fixef))), prob = c((1 - ci) / 2, 1 - (1 - ci) / 2))
    ...
    if (plot == TRUE) {
      ...
      if (predPro == TRUE) {
        names(coef) <-
          c(var2, paste0("values_in_", var1), "coef", "ub", "lb")
      } else {
        names(coef) <- c(var2, "coef", "ub", "lb")
      }
